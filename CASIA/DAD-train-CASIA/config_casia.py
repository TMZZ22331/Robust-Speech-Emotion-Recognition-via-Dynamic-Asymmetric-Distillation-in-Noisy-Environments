...
"""
CASIA数据集跨域任务配置文件
从预训练的CASIA权重开始，在20db噪声环境下进行跨域测试

🎯 任务设置：
   源域：CASIA干净数据 (预训练权重)
   目标域：CASIA 20db噪声数据 (跨域测试)
   
📊 数据配置：
   干净数据：processed_features_CASIA_clean
   噪声数据：processed_features_CASIA_noisy/processed_features_noisy_20db
...
# === 预训练权重配置 ===
PRETRAINED_CASIA_WEIGHT = r"C:\Users\admin\Desktop\111\good_-emo\CASIA\pretrain-and-processed-CASIA\train_for_clean_models\casia_loso_20250731_173528\best_model_fold_4.ckpt"
...
ECDA_START_EPOCH = 30         # [修改] ECDA 启动轮次, 在教师网络稳定后再启动
...
DACP_QUANTILE_START = 0.40       # 动态筛选标准起始值 q_start
DACP_QUANTILE_END = 0.80         # 动态筛选标准终止值 q_end

# 阶段四：最终阈值生成 (公式 15, 17)
DACP_CALIBRATION_STRENGTH_LAMBDA = 0.1 # 类别表现影响大小的校准强度 λ
DACP_THRESHOLD_SMOOTHING_ALPHA = 0.9   # 最终阈值平滑系数 α

# === 消融实验和模块控制 ===
USE_DACP = False                # [消融开关] 是否启用DACP动态阈值
USE_ECDA = False               # [消融开关] 是否启用ECDA分布对齐
...
ECDA_COMPACTNESS_WEIGHT_GAMMA = 0.05 # 紧凑性损失权重 γ (降低以增加稳定性)
ECDA_REPULSION_WEIGHT_DELTA = 0.05  # 类间斥力损失权重 δ

# 损失权重
WEIGHT_CONSISTENCY = 1.0      # 一致性损失权重
TARGET_SCL_WEIGHT = 0.0       # SCL损失权重 (设置为0)
WEIGHT_ECDA = 0.35             # 新的ECDA损失权重 (λ_ECDA)

# === 训练配置 ===
EPOCHS = 500
LEARNING_RATE = 5e-4          # 跨域任务使用更低学习率
WEIGHT_DECAY = 1e-5
...
# 结果保存目录
MODEL_SAVE_DIR = "casia_cross_domain_models"
LOG_DIR = "casia_cross_domain_logs"
RESULTS_DIR = "casia_cross_domain_results"
...
def validate_config():
    ...
    if not os.path.exists(CLEAN_DATA_DIR):
        print(f"⚠️ 警告: CASIA干净数据目录不存在: {CLEAN_DATA_DIR}")
    
    if not os.path.exists(NOISY_DATA_DIR):
        print(f"⚠️ 警告: CASIA噪声数据目录不存在: {NOISY_DATA_DIR}")
    
    # 检查预训练权重
    if not os.path.exists(PRETRAINED_CASIA_WEIGHT):
        print(f"⚠️ 警告: CASIA预训练权重不存在: {PRETRAINED_CASIA_WEIGHT}")
    ...
    assert NUM_CLASSES == 4, "CASIA数据集应有4个类别"
    assert len(LABEL_DICT) == NUM_CLASSES, "标签字典长度与类别数不匹配"
    
    print("✅ CASIA跨域配置参数验证完成")
...
def print_config():
    ...
    print(f"   💾 预训练权重: {PRETRAINED_CASIA_WEIGHT}")
