...
# [删除] 移除不再使用的 SupervisedContrastiveLoss
...
class DACPManager:
    """
    [cite_start]动态自适应置信度剪枝 (DACP) 管理器 [cite: 7]
    [cite_start]实现论文 3.3 节的完整逻辑 [cite: 33]
    """
    def __init__(self, num_classes, total_epochs, device):
        ...
        # 阶段二：类别表现追踪 (公式 10)
        # [cite_start]每个类别的伪标签质量分数 Q_c^e, 初始化为0.5 [cite: 45, 47]
        self.class_quality_scores = torch.full((num_classes,), 0.5, device=self.device)

        # 阶段四：最终阈值平滑 (公式 17)
        # [cite_start]每个类别的最终阈值 τ_c^t, 初始化为0.5 [cite: 70, 71]
    ...
    def calculate_certainty_scores(probs):
        """
        [cite_start]阶段一：计算修正后的确定性分数 (公式 8) [cite: 37, 41]
        Args:
    ...
    def update_class_quality_scores_epoch(self, epoch_scores_per_class):
        """
        [cite_start]阶段二：更新每个类别的整体质量分数 Q_c^e (公式 9, 10) [cite: 44]
        应在每个 epoch 结束后调用
        """
        ...
        # [cite_start]EMA 更新 (公式 10) [cite: 47]
    ...
    def calculate_mask(self, teacher_probs, epoch, calibrated_anchors):
        """
        [cite_start]计算最终的置信度掩码 (DACP 核心逻辑) [cite: 7]
        Args:
            teacher_probs: 教师网络输出的概率 [B, C]
            epoch: 当前轮次
            [cite_start]calibrated_anchors: 预先计算好的动态校准锚点 [C] [cite: 12]
        Returns:
            [cite_start]mask: 高置信度样本的二值掩码 [B] [cite: 72]
        ...
        # [cite_start]--- 阶段三 & 四 开始 --- [cite: 50, 62]
        # [cite_start]1. 计算相对表现差距 Δ_c^e 和类别权重 W_c^e (公式 11, 12) [cite: 51]
        avg_quality = self.class_quality_scores.mean()
        # [cite_start]delta_ce = self.class_quality_scores - avg_quality # [cite: 52]
        delta_ce = self.class_quality_scores - avg_quality
        # [cite_start]使用 sigmoid 计算类别权重 (公式 12) [cite: 54]
        class_weights_wce = torch.sigmoid(cfg.DACP_SENSITIVITY_K * delta_ce)

        # [cite_start]2. 计算动态筛选标准 γ_e (公式 13) [cite: 57]
        progress = epoch / self.total_epochs
        gamma_e = cfg.DACP_QUANTILE_START + (cfg.DACP_QUANTILE_END - cfg.DACP_QUANTILE_START) * progress
        
        # [cite_start]3. 计算批次内各类别基础阈值 τ_c^t (公式 14) [cite: 60]
        ...
        # [cite_start]dynamic_adjustment = cfg.DACP_CALIBRATION_STRENGTH_LAMBDA * (class_weights_wce - 0.5) # [cite: 65]
...
class ECDALoss(nn.Module):
    """
    [cite_start]基于能量最小化的类别感知分布对齐 (ECDA) 损失 [cite: 76]
    [cite_start]实现论文 3.4 节的完整逻辑 [cite: 79]
    """
    ...
    def _gaussian_kernel(self, source, target, sample_weights_s, sample_weights_t):
        ...
        # [cite_start]Term_noisy (公式 22) [cite: 96]
        term_tt = (K_tt * weights_t_matrix).sum() / (weights_t_matrix.sum() + 1e-8)
        # [cite_start]Term_cross (公式 23) [cite: 98]
    ...
    def forward(self, clean_feats, noisy_feats, clean_labels, noisy_labels, noisy_mask, noisy_scores, class_weights_wce):
        ...
        # [cite_start]计算所有含噪特征簇的质心 (用于类间斥力) [cite: 108]
        noisy_centroids = []
        ...
        # [cite_start]计算类间斥力损失 (公式 27) [cite: 114]
        ...
        # [cite_start]计算类别级注意力权重 (公式 24) [cite: 90, 102]
        ...
        for c in range(num_classes):
            # [cite_start]1. 提取每个类别的特征 (公式 19) [cite: 84]
            ...
            # [cite_start]计算可行性门控 (论文 3.4 阶段二) [cite: 100]
            if len(class_clean_feats) < 2 or len(class_noisy_feats) < 2:
                continue

            # [cite_start]2. 注意力加权MMD (公式 20) [cite: 88, 92]
            clean_weights = torch.ones(len(class_clean_feats), device=clean_feats.device)
            # [cite_start]noisy_weights = noisy_scores[noisy_class_mask] # 样本级注意力 [cite: 89]
            ...
            # [cite_start]mmd_attn_loss = term_ss + term_tt - 2 * term_st # [cite: 85]
            mmd_attn_loss = term_ss + term_tt - 2 * term_st

            # [cite_start]3. 分布紧凑性正则化 (公式 25) [cite: 105, 107]
            # [cite_start]noisy_centroid = class_noisy_feats.mean(dim=0) # [cite: 108]
            ...
            # [cite_start]4. 最终对齐损失 (公式 28) [cite: 116]
            ...
            # [cite_start]5. 总损失加权求和 (公式 29) [cite: 119]
