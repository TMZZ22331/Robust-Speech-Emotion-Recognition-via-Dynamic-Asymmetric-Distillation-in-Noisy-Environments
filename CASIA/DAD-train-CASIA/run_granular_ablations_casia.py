...
def run_single_experiment(experiment_name, config_overrides, fold):
    ...
    """
    logger.info(f"--- 🚀 开始CASIA消融实验: {experiment_name} (Fold {fold+1}) ---")
    
    # --- 动态加载和修改配置 ---
    import config_casia as config
    ...
    # --- 运行训练 ---
    import train_CASIA as train
    importlib.reload(train)
    
    trainer = train.FixedCASIACrossDomainTrainer(fold=fold, experiment_name=experiment_name)
...
def main():
    """主函数，定义并运行所有CASIA细粒度消融实验"""
    
    # --- 实验配置 ---
    # CASIA 使用第4折 (fold=3)，测试四种噪声类型的10dB条件
    TARGET_FOLD = 3  # CASIA默认使用第4折
    BASE_NOISE_PATH = r"C:\Users\admin\Desktop\DATA\processed_features_CASIA_real_noise"
    
    # 四种噪声类型的10dB数据路径
    NOISE_TYPES = {
        'babble': f"{BASE_NOISE_PATH}\\processed_features_babble_10db",
        # 'factory': f"{BASE_NOISE_PATH}\\processed_features_factory_10db", 
        # 'hfchannel': f"{BASE_NOISE_PATH}\\processed_features_hfchannel_10db",
        # 'volvo': f"{BASE_NOISE_PATH}\\processed_features_volvo_10db"
    }
    ...
    granular_ablations = [
        ...
        #         'ECDA_COMPACTNESS_WEIGHT_GAMMA': 0.05,
        #         'ECDA_REPULSION_WEIGHT_DELTA': 0.05
        #     }
        # },
    ...
    all_results = {}
    logger.info(f"\n{'='*80}")
    logger.info(f"🚀 开始CASIA细粒度消融实验 (Fold {TARGET_FOLD+1}, 10dB噪声) 🚀")
    logger.info(f"📊 将在 {len(NOISE_TYPES)} 种噪声类型上测试: {list(NOISE_TYPES.keys())}")
    logger.info(f"{'='*80}")
    ...
    # 保存结果到CSV文件
    csv_path = f'casia_granular_ablation_results_fold_{TARGET_FOLD+1}_10db.csv'
    ...
    if detailed_data:
        ...
        # 保存详细表格
        detailed_csv_path = f'casia_detailed_noise_breakdown_fold_{TARGET_FOLD+1}_10db.csv'
