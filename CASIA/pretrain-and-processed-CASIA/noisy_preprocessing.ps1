...
#运行命令-CASIA_ROOT"your path" -output_base_path "your path" -snr_levels @(20)
#.\noisy_preprocessing.ps1 -CASIA_ROOT "C:\Users\admin\Desktop\DATA\CASIA\CASIA情感语料库" -output_base_path "C:\Users\admin\Desktop\DATA\processed_features_CASIA_noisy" -snr_levels @(20,15,10,0)
Write-Host "========================================" -ForegroundColor Green
Write-Host "Multi-SNR CASIA Noisy Audio Preprocessing Pipeline" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
...
foreach ($snr_db in $snr_levels) {
    ...
    try {
        ...
        if (Test-Path $addNoiseScript) {
            ...
            $cmd = "& python `"$addNoiseScript`" --input_root `"$CASIA_ROOT`" --output_root `"$noisyAudioPath`" --snr_db $snr_db --manifest_path `"$output_path\train.tsv`""
...
Write-Host ""
Write-Host "========================================" -ForegroundColor Green
Write-Host "Multi-SNR CASIA Preprocessing Pipeline Completed!" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
