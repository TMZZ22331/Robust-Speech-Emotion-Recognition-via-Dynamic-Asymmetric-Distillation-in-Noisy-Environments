<#
.SYNOPSIS
    用于CASIA情感语料库的统一预处理流水线。
    支持干净数据和带噪数据的特征提取。

.DESCRIPTION
    该脚本完成以下任务:
    1. 调用 casia_manifest.py 为原始音频生成 manifest, label, 和 speaker 文件。
    2. (可选) 如果指定了 -AddNoise, 则会为每个SNR级别循环执行以下操作:
        a. 创建一个临时目录，并使用 add_noise_to_audio.py 生成带噪声音频。
        b. 调用 casia_manifest_noisy.py 为带噪声音频生成新的 manifest。
    3. 调用 emotion2vec_speech_features.py 提取特征。
    4. 清理临时文件。

.PARAMETER CASIA_ROOT
    CASIA情感语料库的根目录的绝对路径。
    例如: "C:\Users\admin\Desktop\DATA\CASIA\CASIA情感语料库"

.PARAMETER OutputBasePath
    用于存放所有输出文件的基础目录。
    脚本会在此目录下为每个处理任务创建子目录。
    例如: "C:\Users\admin\Desktop\DATA\CASIA_processed"
...
.EXAMPLE
    # 示例1: 只处理干净数据
    .\casia_preprocessing.ps1 -CASIA_ROOT "C:\path\to\CASIA" -OutputBasePath "C:\path\to\output"
    .\casia_preprocessing.ps1 -CASIA_ROOT "C:\Users\admin\Desktop\DATA\CASIA\CASIA情感语料库" -OutputBasePath "C:\Users\admin\Desktop\DATA\meihua\CASIA"
    .\casia_preprocessing.ps1 -CASIA_ROOT "C:\Users\admin\Desktop\DATA\CASIA\CASIA情感语料库" -OutputBasePath "C:\Users\admin\Desktop\三个数据集的代码美化版本\CASIA\pretrain-and-processed-CASIA\data"

.EXAMPLE
    # 示例2: 处理多种信噪比的噪声数据 (0dB, 5dB, 10dB)
    .\casia_preprocessing.ps1 -CASIA_ROOT "C:\Users\admin\Desktop\DATA\CASIA\CASIA情感语料库" -OutputBasePath "C:\Users\admin\Desktop\DATA\meihua\CASIA" -AddNoise -SnrLevels @(20)
#>
param(
...
# --- 脚本路径检查 ---
$casia_manifest_script = Join-Path $base_script_path "scripts\casia_manifest.py"
$casia_manifest_noisy_script = Join-Path $base_script_path "scripts\casia_manifest_noisy.py"
...
$required_scripts = @(
    $casia_manifest_script, $casia_manifest_noisy_script, 
...
if (!$AddNoise) {
    ...
    Write-Host "CASIA Clean Data Preprocessing Pipeline" -ForegroundColor Cyan
    ...
    python $casia_manifest_script --root $CASIA_ROOT --dest $output_path
    ...
    Write-Host "CASIA Noisy Data Preprocessing Pipeline" -ForegroundColor Yellow
    ...
    python $casia_manifest_script --root $CASIA_ROOT --dest $original_manifest_path
...
Write-Host "CASIA Preprocessing Pipeline Completed!" -ForegroundColor Green
Write-Host "Total Duration: $($global_duration.TotalMinutes.ToString('F1')) minutes" -ForegroundColor Green
Write-Host "Generated files are in: $OutputBasePath" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Green 