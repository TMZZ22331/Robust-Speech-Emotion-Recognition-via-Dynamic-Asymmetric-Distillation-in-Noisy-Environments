...
def run_single_experiment(experiment_name, config_overrides, fold):
    ...
    """
    logger.info(f"--- 🚀 开始IEMOCAP消融实验: {experiment_name} (Fold {fold+1}) ---")
...
def main():
    """主函数，定义并运行所有IEMOCAP细粒度消融实验"""
    
    # --- 实验配置 ---
    # IEMOCAP 使用第2折 (fold=1)，测试四种噪声类型的10dB条件
    TARGET_FOLD = 1 
    BASE_NOISE_PATH = r"C:\Users\admin\Desktop\DATA\processed_features_IEMOCAP_noisy"
    ...
    granular_ablations = [
        ...
        # 5. DACP消融：无课程学习
        {
            ...
            'settings': {
                'USE_DACP': True, 
                'USE_ECDA': True,
                'DACP_QUANTILE_START': 0.6,  # 设置为固定值，禁用动态变化
                'DACP_QUANTILE_END': 0.6
            }
        },
    ...
    all_results = {}
    logger.info(f"\n{'='*80}")
    logger.info(f"🚀 开始IEMOCAP细粒度消融实验 (Fold {TARGET_FOLD+1}, 10dB噪声) 🚀")
    logger.info(f"📊 将在 {len(NOISE_TYPES)} 种噪声类型上测试: {list(NOISE_TYPES.keys())}")
    logger.info(f"{'='*80}")
    ...
    # --- 整理并生成结果表格 ---
    logger.info(f"\n{'='*80}")
    logger.info("🎉 所有IEMOCAP细粒度消融实验已完成! 🎉")
    logger.info(f"{'='*80}\n")
    ...
    # 保存结果到CSV文件
    csv_path = f'iemocap_granular_ablation_results_fold_{TARGET_FOLD+1}_10db.csv'
    ...
    if detailed_data:
        ...
        # 保存详细表格
        detailed_csv_path = f'iemocap_detailed_noise_breakdown_fold_{TARGET_FOLD+1}_10db.csv'
