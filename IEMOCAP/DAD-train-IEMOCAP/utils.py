...
class DACPManager:
    ...
    def calculate_certainty_scores(probs):
        ...
        # [消融开关] 根据配置决定是否使用熵增强的置信度分数
        if cfg.USE_ENTROPY_IN_SCORE:
            # 计算熵 (公式 7)
...
class ECDALoss(nn.Module):
    ...
    def forward(self, clean_feats, noisy_feats, clean_labels, noisy_labels, noisy_mask, noisy_scores, class_weights_wce):
        ...
        # [消融开关] 根据配置决定使用类别感知MMD还是全局MMD
        if cfg.USE_CLASS_AWARE_MMD:
            # 原有的类别感知MMD逻辑
            # 计算所有含噪特征簇的质心 (用于类间斥力)
            noisy_centroids = []
            ...
            # [消融实验] 使用全局MMD（忽略类别信息）
            # 提取所有有效的干净特征和噪声特征
            all_clean_feats = clean_feats
            all_noisy_feats = noisy_feats[noisy_mask]
            
            # 确保有足够的样本进行MMD计算
            if len(all_clean_feats) >= 2 and len(all_noisy_feats) >= 2:
                # 为所有样本分配均匀权重
                clean_weights = torch.ones(len(all_clean_feats), device=clean_feats.device)
                noisy_weights = torch.ones(len(all_noisy_feats), device=clean_feats.device)
                
                # 计算全局MMD
                term_ss, term_tt, term_st = self._gaussian_kernel(all_clean_feats, all_noisy_feats, clean_weights, noisy_weights)
                global_mmd_loss = term_ss + term_tt - 2 * term_st
                
                total_loss = global_mmd_loss
            # 注意：当使用全局MMD时，紧凑性损失和斥力损失应该被禁用（通过配置参数设置为0）
