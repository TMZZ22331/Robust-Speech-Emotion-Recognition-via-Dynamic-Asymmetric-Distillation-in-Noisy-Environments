...
def load_noise_files(noise_root):
    ...
    if not os.path.exists(noise_root):
        raise FileNotFoundError(f"❌ 噪声目录不存在: {noise_root}")
    ...
    for filename, noise_type in NOISE_FILE_MAPPING.items():
        ...
        if os.path.exists(file_path):
            ...
            print(f"  ✅ Found {noise_type}: {filename}")
        else:
            missing_files.append(filename)
            print(f"  ❌ Missing {noise_type}: {filename}")
    
    # 如果有缺失的噪声文件，直接报错
    if missing_files:
        raise FileNotFoundError(
            f"❌ 缺失必需的噪声文件: {missing_files}\n"
    ...
    if len(found_types) != 5:
        raise ValueError(
            f"❌ 噪声类型数量不匹配! 期望5种，实际找到{len(found_types)}种\n"
            f"期望类型: {expected_types}\n"
            f"找到类型: {found_types}"
        )
    
    print(f"✅ 成功加载所有5种噪声类型，共 {sum(len(files) for files in noise_files.values())} 个文件")
    ...
    try:
        ...
        if noise_mode == "random":
            ...
            if noise_type not in noise_files:
                ...
                raise ValueError(
                    f"❌ 噪声类型 '{noise_type}' 不存在!\n"
                    f"可用的噪声类型: {available_types}\n"
                    f"请检查噪声类型名称是否正确。\n"
                    f"支持的噪声类型: babble, f16, factory, hfchannel, volvo"
                )
...
def get_parser():
    ...
    parser.add_argument(
        "--noise_type",
        help="Specific noise type to use (only for type_specific mode)"
    )
    return parser


def main(args):
    print(f"Adding real noise to IEMOCAP audio files")
