...
def run_single_experiment(experiment_name, config_overrides, fold):
    ...
    """
    logger.info(f"--- 🚀 开始EMODB实验: {experiment_name} (Fold {fold+1}) ---")
    
    # --- 动态加载和修改配置 ---
    import config_emodb
    importlib.reload(config_emodb)
    
    for key, value in config_overrides.items():
        setattr(config_emodb, key, value)
        logger.info(f"  🔧 设置参数: {key} = {value}")
    
    # --- ⚠️ 关键修复2：更新依赖路径 ---
    if 'NOISY_DATA_DIR' in config_overrides:
        import os
        config_emodb.NOISY_FEAT_PATH = os.path.join(config_emodb.NOISY_DATA_DIR, "train")
        logger.info(f"  📁 噪声特征路径已更新: {config_emodb.NOISY_FEAT_PATH}")
    
    # --- ⚠️ 关键修复1：设置随机种子和环境 ---
    config_emodb.setup_environment()
    logger.info(f"  🎲 随机种子已设置: {config_emodb.RANDOM_SEED}")

    # --- 运行训练 ---
    import train_emodb
    importlib.reload(train_emodb)
    
    trainer = train_emodb.FixedEMODBCrossDomainTrainer(fold=fold, experiment_name=experiment_name)
...
def main():
    ...
    core_ablations = [
        # ... (此处省略了完整模型，因为您已做过)
         {
            ...
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-babble-0db"}
        },
        {
        ...
        },
        {
            ...
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-babble-15db"}
        },
        {
            'name': 'emotion2vec+ssl babble 20db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-babble-20db"}
        },
        {
            'name': 'emotion2vec+ssl factory 0db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-factory-0db"}
        },
        {
            'name': 'emotion2vec+ssl factory 10db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-factory-10db"}
        },
        {
            'name': 'emotion2vec+ssl factory 15db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-factory-15db"}
        },
        {
            'name': 'emotion2vec+ssl factory 20db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-factory-20db"}
        },
        {
            'name': 'emotion2vec+ssl hfchannel 0db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-hfchannel-0db"}
        },
        {
            'name': 'emotion2vec+ssl hfchannel 10db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-hfchannel-10db"}
        },
        {
            'name': 'emotion2vec+ssl hfchannel 15db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-hfchannel-15db"}
        },
        {
            'name': 'emotion2vec+ssl hfchannel 20db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-hfchannel-20db"}
        },
        {
            'name': 'emotion2vec+ssl volvo 0db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-volvo-0db"}
        },
        {
            'name': 'emotion2vec+ssl volvo 10db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-volvo-10db"}
        },
        {
            'name': 'emotion2vec+ssl volvo 15db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-volvo-15db"}
        },
        {
            'name': 'emotion2vec+ssl volvo 20db',
            'settings': {'USE_DACP': False, 'USE_ECDA': False, 'FIXED_CONFIDENCE_THRESHOLD': 0.75, 
                         'NOISY_DATA_DIR': r"C:\Users\admin\Desktop\DATA\processed_features_EMODB_noisy\root1-volvo-20db"}
        },
    ...
    ecda_table_data = {
        ...
        'WA (%)': [full_model_placeholder['WA (%)'], 'N/A', 'N/A'],
        'W-F1 (%)': [full_model_placeholder['W-F1 (%)'], 'N/A', 'N/A']
    }
    ecda_df = pd.DataFrame(ecda_table_data).set_index('模型配置')
    
    print("表2: EMODB ECDA内部模块消融实验")
