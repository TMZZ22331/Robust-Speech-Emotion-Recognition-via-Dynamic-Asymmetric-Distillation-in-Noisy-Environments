#!/usr/bin/env python3
"""
EMODB跨域训练脚本 (DACP+ECDA 增强版)
从预训练的EMODB干净权重开始，在不同噪声环境下进行跨域测试。
- 实现 DACP 和 ECDA 新方法
- 自动识别噪声dB
- 结构化保存结果
- 生成详细报告
...
from dataload_emodb_clean import create_emodb_clean_dataloaders
from dataload_emodb_noisy import create_emodb_noisy_dataloaders_with_speaker_isolation
from utils import (
    DataAugmentation, 
    DACPManager, # 导入新模块
    ECDALoss,    # 导入新模块
...
class FixedEMODBCrossDomainTrainer:
    """
    EMODB跨域训练器 (DACP+ECDA 增强版)
    从预训练的干净权重开始，在噪声环境下进行跨域适应
    """
    
    def __init__(self, fold=0, experiment_name=None):  # 默认使用第1折权重
        """初始化跨域训练器"""
        self.experiment_name = experiment_name
        log_exp_name = f" ({self.experiment_name})" if self.experiment_name else ""
        logger.info(f"🚀 初始化EMODB跨域训练器 (DACP+ECDA 增强版){log_exp_name} (Fold {fold+1}/10)...")
        ...
        logger.info("📋 EMODB跨域训练配置 (DACP+ECDA 增强版):")
        logger.info(f"    - 数据源: EMODB干净数据 → EMODB {self.noise_info['display_name']} 噪声数据")
        logger.info(f"    - 预训练权重: {cfg.PRETRAINED_EMODB_WEIGHT}")
        logger.info(f"    - 结果目录: {self.results_dir}")
        logger.info(f"    - 交叉验证: Fold {fold+1}/10")
    ...
    def _setup_results_directory(self, experiment_name=None):
        ...
        └── root2/
            ...
            └── 15db/
        """
        base_dir = "emodb_mutil-noisy_cross_domain_results"
    ...
    def _setup_model(self):
        """设置模型"""
        logger.info("🏗️ 设置EMODB跨域SSRL模型...")
        self.model = SSRLModel(cfg=cfg).to(self.device)
        if cfg.PRINT_MODEL_INFO:
            total_params = sum(p.numel() for p in self.model.parameters())
            trainable_params = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
            logger.info(f"📊 模型参数: 总计 {total_params:,}, 可训练 {trainable_params:,}")
        logger.info("✅ EMODB跨域SSRL模型设置完成")

    def _load_pretrained_weights(self):
        """加载预训练权重"""
        logger.info(f"💾 加载EMODB预训练权重: {cfg.PRETRAINED_EMODB_WEIGHT}")
        if not os.path.exists(cfg.PRETRAINED_EMODB_WEIGHT):
            logger.error(f"❌ 预训练权重文件不存在!")
            raise FileNotFoundError(f"预训练权重文件不存在: {cfg.PRETRAINED_EMODB_WEIGHT}")
        
        try:
            self.model.load_complete_pretrained_weights(cfg.PRETRAINED_EMODB_WEIGHT)
    ...
    def _run_anchor_calibration(self):
        """[新增] 执行一次性的锚点校准 (论文 3.2 节)"""
        logger.info("⚓️ 正在执行一次性锚点校准 (Anchor Calibration)...")
        # 临时数据加载器，遍历所有数据
        clean_calib_loader, _, _, _, _ = create_emodb_clean_dataloaders(cfg.CLEAN_FEAT_PATH, cfg.BATCH_SIZE * 2, fold=self.fold)
        _, _, noisy_calib_loader, _ = create_emodb_noisy_dataloaders_with_speaker_isolation(cfg.NOISY_FEAT_PATH, cfg.BATCH_SIZE * 2, fold=self.fold)
    ...
    def train_step(self, clean_batch, noisy_batch, epoch):
        ...
        # SCL损失 (如果启用)
        scl_loss = torch.tensor(0.0, device=self.device) # 直接设置为0
        
        if self.is_warmup_phase(epoch):
            total_loss = supervised_ce_loss # 移除SCL
    ...
    def validate(self, data_loader, domain_name="unknown"):
        ...
        return {
            ...
            'precision_per_class': precision_recall_fscore_support(all_labels, all_preds, average=None, zero_division=0)[0].tolist(),
            'recall_per_class': precision_recall_fscore_support(all_labels, all_preds, average=None, zero_division=0)[1].tolist(),
            'f1_per_class': precision_recall_fscore_support(all_labels, all_preds, average=None, zero_division=0)[2].tolist(),
            'support_per_class': precision_recall_fscore_support(all_labels, all_preds, average=None, zero_division=0)[3].tolist(),
            'confusion_matrix': cm
        }

    def check_early_stopping(self, noisy_results, is_best):
        """[增强版] 早停检查 - 核心监控噪声域加权准确率"""
    ...
    def save_checkpoint(self, epoch, clean_results, noisy_results, is_best=False):
        ...
        if is_best:
            ...
            best_path = os.path.join(models_dir, f'emodb_cross_domain_best.pth')
    ...
    def _save_confusion_matrices(self, clean_results, noisy_results, epoch, is_best_result=False):
        ...
        ax1.set_title(f'Clean Domain (Epoch {epoch+1})\nAcc: {clean_results["accuracy"]:.2f}%, Weighted F1: {clean_results["f1_weighted"]:.2f}%')
    ...
    def _save_detailed_results(self, clean_results, noisy_results, epoch, is_best_result=False):
        ...
        def _calculate_per_class_accuracy(cm):
            return [f"{(cm[i][i] / cm[i].sum()):.3f}" if cm[i].sum() > 0 else "0.000" for i in range(len(cm))]
    ...
    def train(self):
        """主训练循环 (增强版)"""
        logger.info(f"🚀 开始EmoDB跨域训练 (Fold {self.fold+1}/5, 噪声: {self.noise_info['display_name']})...")
        ...
        logger.info(f"🎉 EmoDB跨域训练完成 (Fold {self.fold+1})!")
        return {'best_noisy_weighted_acc': self.best_noisy_weighted_acc, 'results_dir': self.results_dir}

    def _evaluate_on_test_set(self):
        """加载最佳模型并在测试集上进行最终评估"""
        best_model_path = os.path.join(self.results_dir, "models", "emodb_cross_domain_best.pth")
...
def main():
    """主函数"""
    logger.info("🚀 启动EMODB跨域训练器 (DACP+ECDA 增强版)...")
    fold = 3
    logger.info(f"📝 EMODB跨域训练 - 使用Fold {fold+1}预训练权重")
    
    trainer = FixedEMODBCrossDomainTrainer(fold=fold)
    results = trainer.train()
    
    logger.info(f"✅ EMODB跨域训练完成!")
