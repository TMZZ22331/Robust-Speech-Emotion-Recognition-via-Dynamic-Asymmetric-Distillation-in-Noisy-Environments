...
def load_dataset(data_path, labels=None, min_length=3, max_length=None):
    ...
    # File paths
    len_path = data_path + ".lengths"
    lbl_path = data_path + f".{labels}" if labels else None
    spk_path = data_path + ".spk"

    # Check existence
    has_labels = lbl_path and os.path.exists(lbl_path)
    has_speakers = os.path.exists(spk_path)

    with open(len_path, "r") as len_f, \
         (open(lbl_path, "r", encoding="utf-8") if has_labels else contextlib.ExitStack()) as lbl_f, \
         (open(spk_path, "r", encoding="utf-8") if has_speakers else contextlib.ExitStack()) as spk_f:
    ...
    # 返回说话人ID，即使为空
    return npy_data, sizes, offsets, emo_labels, speakers
...
def load_ssl_features(feature_path, label_dict, max_speech_seq_len=None):
    ...
    data, sizes, offsets, labels, speakers = load_dataset(data_path, labels='lbl', min_length=1, max_length=max_speech_seq_len)
    ...
    dataset_data = {
        ...
        "offsets": offsets,
        "labels": labels,
        "speakers": np.array(speakers), # 新增说话人数据
        "num": num
    } 
